(() => {
  ...
  function openStudentPanel(){allowStudentOpen=true;window.__PAIBP_STUDENT_GATE_BYPASS__=true;const b=$("[data-open-panel='student']");if(b)b.click();else $("#panel-student,[data-panel='student']")?.removeAttribute("hidden");window.__PAIBP_STUDENT_GATE_BYPASS__=false;allowStudentOpen=false;setTimeout(()=>lockIdentityFields(),50)}
  ...
  function openStudentAccess(){const saved=parse(localStorage.getItem(KEYS.identity),null),lc=linkClass();if(saved?.nisn && /^\d{10}$/.test(String(saved.nisn)) && (!IS_CAT_LINK || normalizeClass(saved.className).code===lc.code)){beginStudent(saved)}else showLogin()}
