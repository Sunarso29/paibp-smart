...
self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;const u=new URL(e.request.url);if(u.origin!==self.location.origin)return;const fresh=e.request.mode==="navigate"||["script","style","document"].includes(e.request.destination)||/app-config|cat-session|uji-server|reset-cache/i.test(u.pathname);if(fresh)e.respondWith(fetch(e.request,{cache:"no-store"}).catch(()=>caches.match(e.request,{ignoreSearch:true})));});
