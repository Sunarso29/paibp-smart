const CACHE_NAME="paibp-smart-v89-fast-static";
const STATIC=[
  "./logo-spensus.png",
  "./assets/icons/icon-192.png",
  "./visual-v86.css?v=86",
  "./icon-v86.css?v=86",
  "./icon-art-v86.js?v=89",
  "./visual-fix-v87.css?v=89",
  "./visual-v89.css?v=89",
  "./home-v89.js?v=89",
  "./teacher-preview-fix-v87.js?v=89",
  "./multimapel-admin-v89.css?v=89",
  "./multimapel-admin-v89.js?v=89"
];
...
self.addEventListener("fetch",event=>{const request=event.request;if(request.method!=="GET")return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(request.mode==="navigate"||request.destination==="document"){event.respondWith(networkFirst(request));return;}if(["86","87","88","89"].includes(url.searchParams.get("v"))&&(request.destination==="script"||request.destination==="style")){event.respondWith(cacheFirst(request));return;}if(request.destination==="image"||request.destination==="font"){event.respondWith(cacheFirst(request));return;}event.respondWith(networkFirst(request));});
