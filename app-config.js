window.PAIBP_CONFIG = Object.freeze({
  ...
  aiEnabled: true,
  realtimeManagedBy: "v70-stable-worker",
  realtimeEndpoint: "",
  realtimeReadKey: ""
});
...
(() => {
  ...
  function idle(task,timeout=2000){if("requestIdleCallback" in window)requestIdleCallback(task,{timeout});else setTimeout(task,Math.min(timeout,900))}

  style("performance-v65.css");
  style("mobile-fix-v70.css");
  script("stability-v70.js").catch(()=>{});
  if (page === "about-spensus.html") return;

  const lowMemory = Number(navigator.deviceMemory || 8) <= 4 || matchMedia("(max-width:760px)").matches;
  const corePages = /^(index|akses-guru|kendali-editor)\.html$/.test(page);
  if (corePages) {
    style("cat-session-v65.css");
    script("stability-v70.js").then(()=>script("cat-session-v67.js")).catch(()=>{});
  }
  ...
  if(!lowMemory) idle(loadLegacy,3000);
  else document.addEventListener("click",function onDemand(event){
    if(event.target.closest('[data-chapter],[data-material-id],[data-practice],[data-open-practice],#v56-practice-board')){
      document.removeEventListener("click",onDemand,true);loadLegacy();
    }
  },true);
  ...
  document.addEventListener("click",(event)=>{
    ...
    if(event.target.closest('[data-chapter],[data-material-id],[data-practice],[data-open-practice],#v56-practice-board')){
      mediaRequested=true;script("media-pack-v56.js").catch(()=>{});
    }
  },{passive:true});
  ...
  document.addEventListener("click",(event)=>{
    if(event.target.closest("[data-ai-open],.workspace-ai-nav-v27")){
      style("spensus-ai-v48.css");script("spensus-ai-v56.js").catch(()=>{});
    }
  },{passive:true});

  if(localStorage.getItem("paibp-smart-v70-cache-reset")!=="done")setTimeout(async()=>{
    try{
      const keys=await caches.keys();await Promise.all(keys.map((key)=>caches.delete(key)));
      localStorage.setItem("paibp-smart-v70-cache-reset","done");
