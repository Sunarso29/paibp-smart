window.PAIBP_CONFIG = Object.freeze({
  ...
  aiEnabled: true,
  realtimeManagedBy: "v65-final-classroom",
  realtimeEndpoint: "",
  realtimeReadKey: ""
});
...
(() => {
  ...
  if (corePages) {
    ...
    script("cat-session-v65.js").catch(()=>{});
  }
  ...
  idle(()=>script("realtime-v56.js").catch(()=>{}),4200);
  ...
  if(localStorage.getItem("paibp-smart-v65-cache-reset")!=="done")setTimeout(async()=>{
    try{
      const keys=await caches.keys();await Promise.all(keys.map((key)=>caches.delete(key)));
      localStorage.setItem("paibp-smart-v65-cache-reset","done");
