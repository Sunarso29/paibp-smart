(() => {
  ...
  const KEYS = Object.freeze({
    ...
    session: "paibp-smart-student-session-v65",
    teacher: "paibp-smart-teacher-identity-v1",
    teacherScope: "paibp-smart-teacher-scope-v65",
    authority: "paibp-smart-authority-v56",
    editor: "paibp-smart-editor-unlocked",
    policyCache: "paibp-smart-policy-cache-v65",
  ...
  function jsonpOnce(action, params = {}, timeout = 6500) {
    return new Promise((resolve,reject) => {
      ...
      const callback = `paibpV65_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  async function serverCall(action, params = {}) {
    try { return await jsonpOnce(action, params, 5000); }
    catch (first) {
  ...
  async function getClassStatus(classCode) {
    const result = await serverCall("classStatusV65",{classCode});
  ...
  function ensureTeacherControl() {
    ...
    async function command(commandName) {
      ...
      const result = await serverCall("classControlV65",{
  ...
  async function refreshTeacherClasses() {
    ...
    const status = $("[data-v65-control-status]",panel);
    try {
      const result = await serverCall("classListV65",{readKey:READ_KEY,actorScope:teacherScope(),actorRole:isEditor()?"editor":"teacher"});
