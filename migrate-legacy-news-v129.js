...
function mediaSources(x){
  const out=[];
  const add=v=>{if(!v)return;const s=typeof v==="string"?v:(v.src||v.full||v.thumbnail||v.url||"");if(s&&!out.includes(s))out.push(s)};
...
async function compressDataUrl(src){
  ...
  try{
    ...
    const bmp=await createImageBitmap(blob);let best="";
...
async function migrateOne(x,index,total){
  ...
  const sources=mediaSources(x),photoKeys=[];
  ...
  await post("contentUpsert",{key:`news:${id}`,value:{schema:"chunks-v102",id,title,date,category:x.category||"Berita Sekolah",summary,photoKeys,bodyKeys,coverKey:photoKeys[0]||"",year:Number(x.year||date.slice(0,4))||0,month:Number(x.month||date.slice(5,7))||0,author:x.author||"Sunarso, S.Pd.I, Gr",migratedFrom:"legacy-browser-v129"},authorName:x.author||"Sunarso, S.Pd.I, Gr",authorSchool:"SMP Negeri 1 Susukan"});
  return true;
}
async function run(){
  if(localStorage.getItem(DONE_KEY)){go();return}
  const [local,cache]=await Promise.all([localPosts(),Promise.resolve(cachedPosts())]);
  const posts=mergePosts(local,cache);
