(() => {
  ...
  const FOCUS_KEY = "paibp-smart-focus-session-v48";
  ...
  function toast(message, tone = "warning") {
    let node = $("#v48-learning-toast");
    if (!node) {
      node = document.createElement("div");
      node.id = "v48-learning-toast";
      node.className = "v48-learning-toast";
  ...
  function decorate(card, chapter, reason) {
    if (!card) return;
    card.classList.toggle("v48-chapter-locked", Boolean(reason));
    $(".v44-chapter-lock-notice", card)?.remove();
    $(".v45-chapter-lock-notice", card)?.remove();
    let notice = $(".v48-chapter-lock-notice", card);
    if (!reason) { notice?.remove(); return; }
    if (!notice) {
      notice = document.createElement("div");
      notice.className = "v48-chapter-lock-notice";
      card.append(notice);
    }
  ...
  function applyLocks() {
    ...
    grouped.forEach((buttons, id) => {
      ...
      buttons.forEach((button) => {
        ...
        button.classList.toggle("v48-button-locked", Boolean(reason));
  ...
  function ensureFocusGate() {
    let gate = $("#v48-focus-gate");
    if (gate) return gate;
    gate = document.createElement("div");
    gate.id = "v48-focus-gate";
    gate.className = "v48-focus-gate";
    gate.hidden = true;
    gate.innerHTML = `<section role="dialog" aria-modal="true"><span class="v48-focus-kicker">MODE BELAJAR TERARAH</span><h2>Belajar runtut dalam satu ruang</h2><p data-v48-focus-message>Bab dibuka berurutan. Tautan luar dibatasi selama sesi belajar dan progres tetap tersimpan.</p><div><span>✓ Bab berurutan</span><span>✓ Progres tersimpan</span><span>✓ Keluar halaman tercatat</span></div><button type="button" data-v48-enter-focus>Mulai Belajar Full Screen</button><small>Browser tetap menyediakan tombol keselamatan untuk keluar dari layar penuh.</small></section>`;
    document.body.append(gate);
    $("[data-v48-enter-focus]", gate).addEventListener("click", enterFocus);
    return gate;
  }
  ...
  async function enterFocus() {
    ...
    document.documentElement.classList.add("v48-student-focus");
  }

  function pauseFocus(message) {
    if (!studentActive) return;
    focusActive = false;
    document.documentElement.classList.remove("v48-student-focus");
    const gate = ensureFocusGate();
    $("[data-v48-focus-message]", gate).textContent = message;
    gate.hidden = false;
  }
  ...
  function renderEditorPanel() {
    ...
    if (!panel) {
      ...
      panel.id = "v48-editor-chapter-control";
      panel.className = "v48-editor-chapter-control";
    ...
    $$('[data-v48-lock]', panel).forEach((input) => input.addEventListener("change", () => input.closest("label")?.classList.toggle("is-locked", input.checked)));
    $("[data-v48-save]", panel).addEventListener("click", () => {
      const mode = $('[data-v48-mode][aria-pressed="true"]', panel)?.dataset.v48Mode === "open" ? "open" : "sequential";
      const next = normalizePolicy({
        mode,
        forceLocked: $$('[data-v48-lock]:checked', panel).map((input) => input.dataset.v48Lock),
