(() => {
  ...
  const WORK_PREFIX = "spensus-multimapel-v36-";
  ...
  function filtersHtml(prefix) {
    return `<div class="multi-filter-v36">
      ...
      <label class="multi-search-v36"><span>Cari bab atau topik</span><input data-mm-query="${prefix}" type="search" placeholder="Contoh: bilangan berpangkat, teks berita, sel…"></label>
    </div>`;
  }
  ...
  function cardsHtml(items) {
    if (!items.length) return `<div class="multi-empty-v36"><strong>Materi belum ditemukan.</strong><p>Ubah kelas, semester, mata pelajaran, atau kata pencarian.</p></div>`;
    return items.map((item) => {
      ...
      return `<article class="multi-module-card-v36" style="--subject:${escapeHtml(subject.color)}">
        <div class="multi-card-visual-v36"><span class="multi-card-icon-v36">${escapeHtml(subjectIcon(item))}</span><span class="multi-card-number-v36">${String(item.number).padStart(2, "0")}</span></div>
        <div class="multi-card-copy-v36"><span>${escapeHtml(item.subjectName)} • Kelas ${escapeHtml(item.grade)}</span><h3>${escapeHtml(title)}</h3><p>Semester ${escapeHtml(item.semester)} • materi, latihan, ringkasan, video, dan LKPD.</p></div>
        <div class="multi-card-actions-v36"><button type="button" data-mm-open="${escapeHtml(item.id)}">Buka paket belajar <b>→</b></button><a href="mapel-${escapeHtml(item.subject)}.html" aria-label="Buka ringkasan ${escapeHtml(item.subjectName)}">Ringkasan mapel</a></div>
      </article>`;
    }).join("");
  }
  ...
  function originalCoreHtml(item) {
    ...
    return groups.map((group, index) => group.length ? `<section class="multi-core-block-v36"><h4><span>${index + 1}</span>${labels[index]}</h4>${group.map((text, subIndex) => `<div class="multi-subpoint-v36"><b>${String.fromCharCode(97 + subIndex)}.</b><p>${escapeHtml(text)}</p></div>`).join("")}</section>` : "").join("");
  }
  ...
  function materialHtml(item) {
    ...
    const topic = item.title.replace(/^Bab\s+\d+\s*:\s*/i, "").replace(/^Unit\s+\d+\s*:\s*/i, "");
    return `<article class="multi-document-v36 multi-material-v36" data-mm-panel="material">
      <header class="multi-chapter-cover-v36"><div><span>${escapeHtml(item.subjectName)} • Kelas ${escapeHtml(item.grade)} • Semester ${escapeHtml(item.semester)}</span><b>${escapeHtml(item.title.split(":")[0])}</b><h2>${escapeHtml(topic)}</h2><p>Materi terstruktur • latihan langsung • pendalaman • video penguatan • ringkasan minimal 500 karakter</p></div><div class="multi-cover-symbol-v36">${escapeHtml(subjectIcon(item))}</div></header>
      <section class="multi-numbered-section-v36"><h3><span>A</span>Tujuan Pembelajaran</h3><ol class="multi-decimal-list-v36">${(item.objectives || []).map((objective) => `<li>${escapeHtml(stripHtml(objective))}</li>`).join("")}</ol></section>
      <section class="multi-numbered-section-v36"><h3><span>B</span>Peta Konsep</h3><div class="multi-concept-map-v36">${chips.map((chip, index) => `<span><b>${index + 1}</b>${escapeHtml(chip)}</span>`).join("")}</div></section>
      <section class="multi-numbered-section-v36"><h3><span>C</span>Uraian Materi Inti</h3>${originalCoreHtml(item)}</section>
      <section class="multi-numbered-section-v36 multi-direct-practice-v36"><h3><span>D</span>Latihan Pemahaman Langsung</h3><p class="multi-section-intro-v36">Jawablah setelah membaca uraian. Jawaban tersimpan otomatis dan dapat dikirim sebagai satu paket kepada guru.</p>${questions.map((question, index) => `<label class="multi-question-v36"><b>${index + 1}</b><span>${escapeHtml(question)}</span><textarea rows="4" data-mm-field="answer-${index}" placeholder="Tuliskan jawaban yang runtut dan berbukti…"></textarea></label>`).join("")}</section>
      <section class="multi-numbered-section-v36 multi-deep-task-v36"><h3><span>E</span>Latihan Mendalam</h3><div class="multi-deep-grid-v36"><article><b>Situasi</b><p>Pilih satu persoalan nyata di sekolah, keluarga, lingkungan, atau ruang digital yang dapat dianalisis menggunakan materi “${escapeHtml(topic)}”.</p></article><article><b>Tantangan</b><p>Susun bukti, alasan, langkah penyelesaian, dan ukuran keberhasilan. Hindari jawaban yang hanya berupa pendapat tanpa dasar.</p></article></div><label class="multi-block-field-v36">Analisis dan rancangan solusi<textarea rows="8" data-mm-field="deep-task" placeholder="Tuliskan masalah, data/bukti, analisis, alternatif solusi, pilihan tindakan, dan refleksi…"></textarea></label></section>
      <section class="multi-numbered-section-v36"><h3><span>F</span>Video Penguatan</h3><p class="multi-section-intro-v36">Tautan video diarahkan ke pencarian sumber pendidikan relevan. Video memerlukan internet dan tidak disalin ke repository agar website tetap ringan serta menghormati hak cipta.</p><div class="multi-video-grid-v36">${videoCards(item)}</div><label class="multi-block-field-v36">Ringkasan video pilihan — minimal 500 karakter<textarea rows="8" minlength="500" data-mm-field="video-summary" placeholder="Catat gagasan utama, bukti, hubungan dengan materi, dan hal yang masih perlu dipelajari…"></textarea><small data-mm-char-count="video-summary">0 karakter • minimal 500</small></label></section>
      <section class="multi-numbered-section-v36 multi-summary-panel-v36"><h3><span>G</span>Ringkasan Bab</h3><p>${escapeHtml(narrative)}</p><div class="multi-summary-count-v36">${narrative.length.toLocaleString("id-ID")} karakter</div></section>
      <section class="multi-numbered-section-v36 multi-submit-v36"><h3><span>H</span>Simpan dan Kirim kepada Guru</h3>${identityFields()}<div class="multi-submit-actions-v36 no-print"><button type="button" data-mm-save>💾 Simpan</button><button type="button" data-mm-docx>📄 Unduh DOCX</button><button class="is-primary" type="button" data-mm-send>${svg.send} Kirim kepada guru</button></div><p data-mm-status class="multi-status-v36" aria-live="polite"></p></section>
    </article>`;
  }
  ...
  function summaryHtml(item) {
    const narrative = summaryNarrative(item);
    const points = cleanSummaryPoints(item);
    return `<article class="multi-document-v36 multi-summary-v36" data-mm-panel="summary" hidden><header><span>RINGKASAN PROFESIONAL</span><h2>${escapeHtml(item.title)}</h2><p>${narrative.length.toLocaleString("id-ID")} karakter • dapat dipelajari mandiri</p></header><section class="multi-summary-lead-v36"><p>${escapeHtml(narrative)}</p></section><section class="multi-summary-points-v36"><h3>Pokok Materi</h3><ol>${points.map((point) => `<li>${escapeHtml(point)}</li>`).join("")}</ol></section><section class="multi-summary-recall-v36"><h3>Cek Ingatan Cepat</h3><div>${directQuestions(item).slice(0, 3).map((question, index) => `<label><b>${index + 1}</b><span>${escapeHtml(question)}</span><textarea rows="3" data-mm-field="summary-${index}"></textarea></label>`).join("")}</div></section></article>`;
  }
  ...
  function worksheetHtml(item) {
    ...
    return `<article class="multi-document-v36 multi-lkpd-v36" data-mm-panel="worksheet" hidden><header class="multi-lkpd-cover-v36"><span>LEMBAR KERJA MURID</span><h2>${escapeHtml(item.title)}</h2><p>${escapeHtml(item.subjectName)} • Kelas ${escapeHtml(item.grade)} • Semester ${escapeHtml(item.semester)}</p></header>${identityFields()}<section class="multi-lkpd-instruction-v36"><h3>A. Petunjuk Kerja</h3><ol><li>Baca materi dan sumber secara teliti.</li><li>Gunakan data atau alasan yang dapat diperiksa.</li><li>Kerjakan dengan jujur; diskusi boleh dilakukan sesuai arahan guru.</li><li>Periksa kembali bahasa, perhitungan, sumber, dan kelengkapan jawaban.</li></ol></section><section class="multi-lkpd-activities-v36"><h3>B. Aktivitas Inti</h3>${tasks.map((task, index) => `<label class="multi-lkpd-task-v36"><span>${index + 1}</span><div><p>${escapeHtml(task)}</p><textarea rows="5" data-mm-field="lkpd-${index}" placeholder="Tuliskan hasil kerja secara lengkap…"></textarea></div></label>`).join("")}</section><section class="multi-lkpd-product-v36"><h3>C. Produk dan Presentasi</h3><label class="multi-block-field-v36">Deskripsi produk atau hasil akhir<textarea rows="7" data-mm-field="lkpd-product" placeholder="Jelaskan bentuk produk, proses pembuatan, sumber, pembagian peran, dan hasil…"></textarea></label></section><section class="multi-lkpd-rubric-v36"><h3>D. Rubrik Pemeriksaan Mandiri</h3><table><thead><tr><th>Aspek</th><th>4 — Sangat Baik</th><th>3 — Baik</th><th>2 — Berkembang</th><th>1 — Perlu Bimbingan</th></tr></thead><tbody><tr><th>Ketepatan konsep</th><td>Tepat, lengkap, dan berbukti</td><td>Tepat, ada bagian kecil kurang</td><td>Sebagian tepat</td><td>Belum menunjukkan konsep</td></tr><tr><th>Proses dan penalaran</th><td>Langkah runtut dan alasan kuat</td><td>Langkah cukup runtut</td><td>Langkah belum konsisten</td><td>Proses tidak dijelaskan</td></tr><tr><th>Komunikasi/produk</th><td>Jelas, menarik, dan sesuai tujuan</td><td>Jelas dan sesuai tujuan</td><td>Masih perlu perbaikan</td><td>Belum selesai</td></tr><tr><th>Refleksi</th><td>Jujur, spesifik, dan ada tindak lanjut</td><td>Ada refleksi dan tindak lanjut</td><td>Refleksi masih umum</td><td>Belum ada refleksi</td></tr></tbody></table></section><section class="multi-submit-v36"><div class="multi-submit-actions-v36 no-print"><button type="button" data-mm-save>💾 Simpan LKPD</button><button type="button" data-mm-print>🖨 Cetak / PDF</button><button class="is-primary" type="button" data-mm-send>${svg.send} Kirim kepada guru</button></div><p data-mm-status class="multi-status-v36" aria-live="polite"></p></section></article>`;
  }

  function sourceHtml(item) {
    return `<article class="multi-document-v36 multi-source-v36" data-mm-panel="source" hidden><header><span>${svg.source}</span><div><b>RUJUKAN DAN INTEGRITAS KONTEN</b><h2>${escapeHtml(item.title)}</h2></div></header><section><h3>A. Dokumen Dasar</h3><dl><dt>Perangkat sumber</dt><dd>${escapeHtml(item.sourceName || "Dokumen perangkat pembelajaran terverifikasi")}</dd><dt>Lokasi audit</dt><dd>${escapeHtml(item.sourceOriginal || "Arsip pengembangan multimapel")}</dd><dt>Normalisasi</dt><dd>${escapeHtml(item.note || "Konten disusun ulang untuk kebutuhan murid tanpa identitas guru.")}</dd></dl></section><section><h3>B. Rujukan Resmi</h3><div class="multi-official-links-v36"><a href="https://buku.kemendikdasmen.go.id/katalog/" target="_blank" rel="noopener"><b>SIBI Kemendikdasmen</b><span>Buku teks, buku audio, dan buku interaktif resmi ↗</span></a><a href="https://kurikulum.kemendikdasmen.go.id/panduan-mapel" target="_blank" rel="noopener"><b>Panduan Mata Pelajaran</b><span>Acuan penerapan CP dan pembelajaran mendalam ↗</span></a><a href="https://guru.kemendikdasmen.go.id/kurikulum/referensi-penerapan/capaian-pembelajaran/" target="_blank" rel="noopener"><b>Capaian Pembelajaran</b><span>Referensi CP resmi yang berlaku ↗</span></a></div></section><section class="multi-source-note-v36"><strong>Catatan akademik</strong><p>Materi web mempertahankan pokok isi dokumen sumber. Guru tetap perlu mencocokkan urutan bab, keluasan materi, dan edisi buku dengan buku resmi yang digunakan sekolah.</p></section></article>`;
  }
  ...
  function openModule(root, id) {
    ...
    library.hidden = true;
    viewer.innerHTML = `<div class="multi-viewer-head-v36" style="--subject:${escapeHtml(subjectById[item.subject]?.color || "#087f68")}"><button type="button" data-mm-back>← Kembali</button><div><span>${escapeHtml(item.subjectName)} • Kelas ${escapeHtml(item.grade)} • Semester ${escapeHtml(item.semester)}</span><h1>${escapeHtml(item.title)}</h1></div><div class="multi-viewer-actions-v36"><button type="button" data-mm-docx>DOCX</button><button type="button" data-mm-print>PDF / Cetak</button></div></div><nav class="multi-tabs-v36" aria-label="Bagian paket belajar"><button type="button" aria-pressed="true" data-mm-tab="material">${svg.book} Materi Lengkap</button><button type="button" aria-pressed="false" data-mm-tab="summary">${svg.map} Ringkasan</button><button type="button" aria-pressed="false" data-mm-tab="worksheet">${svg.worksheet} LKPD</button><button type="button" aria-pressed="false" data-mm-tab="source">${svg.source} Rujukan</button></nav><div data-mm-printable>${materialHtml(item)}${summaryHtml(item)}${worksheetHtml(item)}${sourceHtml(item)}</div>`;
  ...
  function initRoot(root) {
    ...
    root.innerHTML = `<div data-mm-library><section class="multi-library-head-v36"><div><span>PORTAL MULTIMAPEL SMP</span><h2>Setiap bab diperlakukan setara dengan PAIBP.</h2><p>Materi terstruktur, latihan langsung, latihan mendalam, video penguatan, ringkasan minimal 500 karakter, LKPD profesional, dan pengiriman tugas kepada guru.</p><div class="multi-head-badges-v36"><b>13 mapel</b><b>${db.moduleCount} paket</b><b>Kelas VII–IX</b><b>Ringan & responsif</b></div></div><strong>${db.moduleCount}<small>paket belajar</small></strong></section>${filtersHtml(prefix)}<p class="multi-count-v36" data-mm-count></p><div class="multi-grid-v36" data-mm-grid></div></div><section data-mm-viewer hidden></section>`;
    bind(root);
    render(root);
