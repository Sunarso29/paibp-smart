  ...
  async function renderRecord(record, records) {
    ...
    try {
      ...
      target.innerHTML = `<section class="v48-cp-head"><div><span>CP LAMA 2025 • SUMBER ASLI</span><h2>${escapeHtml(record.title)}</h2><p>BSKAP Nomor 046/H/KR/2025 • Kelas ${escapeHtml(grade)} • ${escapeHtml(record.originalName)}</p></div><div><a href="${escapeHtml(sourceUrl(record))}" download="${escapeHtml(record.originalName)}">Unduh ${escapeHtml(record.format.toUpperCase())}</a><button type="button" data-v48-cache>Simpan luring</button></div></section>
        ...
        ${body}
        <section class="v48-source-actions"><strong>Dokumen sumber asli</strong><p>Pratinjau ditampilkan langsung tanpa JSZip dan tanpa paket Base64. Berkas sumber baru dimuat ketika diunduh atau disimpan luring.</p><a href="${escapeHtml(sourceUrl(record))}" download="${escapeHtml(record.originalName)}">Unduh ${escapeHtml(record.originalName)}</a><small data-v48-cache-status aria-live="polite"></small></section>`;
