(() => {
  ...
  function renderTable(block, tableIndex) {
    ...
    const rows = filteredRows.map((row, rowIndex) => {
      ...
      sparse.forEach((sourceCell) => {
        ...
        while (cursor < cell.colIndex && cursor < cols) { cells += blankCell(cursor, rowIndex); cursor += 1; }
        if (cursor >= cols) return;
        cells += cell.html;
        cursor = cell.colIndex + cell.span;
      });
      while (cursor < cols) { cells += blankCell(cursor, rowIndex); cursor += 1; }
  ...
  function render(force = false) {
    ...
    try {
      ...
      target.innerHTML = `<section class="v84-source-document v84-doc-${sourceKey.toLowerCase()}" data-v84-source="${signature}"><div class="v84-source-body">${renderBlocks(record)}</div></section>`;
      document.documentElement.dataset.teacherDocV84 = sourceKey.toLowerCase();
      window.PAIBP_TEACHER_PREVIEW_FIX_V87?.sanitize?.();
