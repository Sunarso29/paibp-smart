(() => {
  ...
  function getResponse(raw) {
    ...
    if (/^(halo|hai|assalamualaikum|selamat)/.test(q)) {
      return { html: "<strong>Wa'alaikumussalam, selamat datang.</strong><p>Saya siap membantu mencari materi, menyusun rencana belajar, memberikan latihan singkat, dan mengarahkan layanan portal.</p>" };
    }
    if (q.includes("rencana") || q.includes("jadwal belajar")) {
      return { html: `<strong>Rencana belajar 7 hari</strong><ul><li>Hari 1: pilih satu bab dan pahami tujuan.</li><li>Hari 2: pelajari konsep utama dan dalil.</li><li>Hari 3: tulis ringkasan dengan bahasa sendiri.</li><li>Hari 4: kerjakan latihan dan periksa kesalahan.</li><li>Hari 5: selesaikan LKPD atau aktivitas.</li><li>Hari 6: ulangi bagian yang belum dipahami.</li><li>Hari 7: refleksi dan uji diri tanpa melihat catatan.</li></ul>`, actions:[{label:"Buka materi",action:"panel",value:"student"},{label:"Mainkan game",action:"panel",value:"games"}] };
    }
    if (q.includes("latihan") || q.includes("soal")) {
      ...
      const qs = (sample.questions || []).slice(0,3).map((item) => typeof item === "string" ? item : item?.question || item?.prompt || "Pertanyaan penguatan materi.");
      return { html:`<strong>Latihan singkat — ${escapeHtml(sample.title)}</strong><ol>${qs.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ol><small>Jawab dengan bahasa sendiri, kemudian cocokkan dengan materi bab.</small>`, actions:[{label:"Buka bab terkait",action:"chapter",value:sample.id},{label:"Buka game",action:"panel",value:"games"}] };
    }
    if (q.includes("fitur islami") || q.includes("alquran") || q.includes("al qur'an") || q.includes("dzikir") || q.includes("tajwid") || q.includes("bahasa arab") || q.includes("khutbah")) {
      return { html:"<strong>Fitur Islami tersedia dalam satu ruang.</strong><p>Buka Al Qur'an digital, tajwid praktis, dzikir pagi-petang, Hisnul Muslim, khutbah Jum'at, jadwal sholat, kalender Hijriah, dan Bahasa Arab berjenjang.</p>", actions:[{label:"Buka Fitur Islami",action:"panel",value:"islamic"}] };
    }
    if (q.includes("game") || q.includes("xp") || q.includes("permainan")) {
      return { html:"<strong>Game Edukasi</strong><p>Tersedia kuis, tantangan waktu, XP, lencana, dan progres. Pelajari materi terlebih dahulu agar permainan menjadi penguatan pemahaman.</p>", actions:[{label:"Buka arena",action:"panel",value:"games"}] };
    }
    if (q.includes("guru") || q.includes("editor")) {
      return { html:"Ruang Guru dan Ruang Editor merupakan layanan privat yang tidak ditampilkan pada akses murid atau pengunjung. Hubungi pengelola sekolah untuk akses resmi." };
    }
    ...
    return { html:"Saya belum menemukan kecocokan langsung. Tuliskan kelas, nomor bab, atau tema secara lebih spesifik, misalnya <em>iman kepada kitab Allah kelas VIII</em>.", actions:[{label:"Lihat semua materi",action:"panel",value:"student"},{label:"Fitur Islami",action:"panel",value:"islamic"}] };
  }

  function initInstance(root) {
    const form = root.querySelector("[data-ai-form], #spensus-ai-form");
    const input = root.querySelector("[data-ai-input], #spensus-ai-input");
    const messages = root.querySelector("[data-ai-messages], #spensus-ai-messages");
    if (!form || !input || !messages || root.dataset.aiReady === "yes") return;
    root.dataset.aiReady = "yes";
    ...
    root.querySelectorAll("[data-ai-prompt]").forEach((button) => button.addEventListener("click", () => ask(button.dataset.aiPrompt)));
    root.querySelector("[data-ai-reset], #spensus-ai-reset")?.addEventListener("click", () => {
      messages.innerHTML = "";
      addMessage("assistant", "<strong>Assalamu'alaikum.</strong><p>Saya Spensus AI, asisten pendidikan di PAIBP SMART SMP. Materi apa yang ingin dipelajari hari ini?</p>");
    });
  ...
  document.querySelectorAll(".spensus-ai-instance").forEach(initInstance);
})();
